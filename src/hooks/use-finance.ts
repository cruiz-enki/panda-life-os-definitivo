...
export function useFinance() {
  ...
  const updateCard = async (id: string, patch: Partial<CreditCard>) => {
    ...
    if ('credit_limit' in cleanPatch) cleanPatch.credit_limit = Number(cleanPatch.credit_limit) || 0;
    if ('current_balance' in cleanPatch) cleanPatch.current_balance = Number(cleanPatch.current_balance) || 0;
    if ('min_payment' in cleanPatch) cleanPatch.min_payment = Number(cleanPatch.min_payment) || 0;
    if ('no_interest_payment' in cleanPatch) cleanPatch.no_interest_payment = Number(cleanPatch.no_interest_payment) || 0;
    if ('cut_day' in cleanPatch) cleanPatch.cut_day = Number(cleanPatch.cut_day) || 1;
    if ('payment_day' in cleanPatch) cleanPatch.payment_day = Number(cleanPatch.payment_day) || 20;
