...
export const Route = createFileRoute("/future-letters")({
  component: FutureLettersPage,
});

function FutureLettersPage() {
  ...
  const handleAdd = async () => {
    ...
    const error = await addFutureLetter({
      ...newLetter,
      ...
      <header className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tight flex items-center gap-2">
            <Mail className="w-8 h-8 text-primary" /> Carta del Futuro
          </h1>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input 
                ...
                placeholder="Ej: Para cuando cumpla 30 años, Un mensaje de esperanza..."
              />
            </div>
            <div className="space-y-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-in fade-in duration-300">
          <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto relative shadow-2xl border-primary/20 bg-amber-50 dark:bg-slate-900">
            ...
            <div className="p-8 flex justify-center">
              <Button onClick={() => setViewingLetter(null)} variant="outline" className="rounded-full px-8">
                Cerrar carta
              </Button>
            </div>
          </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {futureLetters.map((letter) => {
          ...
          return (
            <Card 
              key={letter.id} 
              className={`group transition-all duration-500 relative overflow-hidden border-primary/10 ${
                isUnlocked 
                  ? "hover:border-primary/40 hover:shadow-glow cursor-pointer" 
                  : "opacity-80"
              }`}
              onClick={() => {
                if (isUnlocked) {
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                      ...
                      <span>
                        {isUnlocked 
                          ? `Se abrió el ${format(new Date(letter.unlock_date), "d 'de' MMM, yyyy", { locale: es })}`
                          : `Se abrirá el ${format(new Date(letter.unlock_date), "d 'de' MMM, yyyy", { locale: es })}`
                        }
                      </span>
                    </div>
                  </div>
                  ...
                  {!isUnlocked && (
                    <div className="bg-primary/5 rounded-lg p-3 text-[10px] uppercase tracking-widest text-center font-bold text-primary/60">
                      Cerrada · Bajo llave
                    </div>
                  )}
                  {isUnlocked && !letter.is_read && (
                    <div className="bg-emerald-500/10 rounded-lg p-3 text-[10px] uppercase tracking-widest text-center font-bold text-emerald-600 animate-pulse">
                      ¡Ya puedes leerla!
                    </div>
                  )}
                  {isUnlocked && letter.is_read && (
                    <div className="bg-muted rounded-lg p-3 text-[10px] uppercase tracking-widest text-center font-bold text-muted-foreground">
                      Leída
                    </div>
                  )}
                </div>
