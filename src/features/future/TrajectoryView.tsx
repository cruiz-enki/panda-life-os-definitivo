/**
 * **Vista** — Life Trajectory (subvista de /future).
 */
...
export function TrajectoryView() {
  ...
  const [newMetrics, setNewMetrics] = useState({ health: 5, finances: 5, relationships: 5, business: 5, stress: 5 });
  ...
  const getTrajectory = (areaId: string) => {
    ...
    if (last > prev) return isStress ? <TrendingUp className="w-5 h-5 text-rose-500" /> : <TrendingUp className="w-5 h-5 text-emerald-500" />;
    if (last < prev) return isStress ? <TrendingDown className="w-5 h-5 text-emerald-500" /> : <TrendingDown className="w-5 h-5 text-rose-500" />;
    return <Minus className="w-5 h-5 text-muted-foreground" />;
  };

  const chartData = lifeMetrics.map(m => ({ ...m, date: format(new Date(m.created_at), "d MMM", { locale: es }) }));
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-2">
            <Activity className="w-7 h-7 text-primary" /> Life Trajectory
          </h2>
          <p className="text-muted-foreground text-sm mt-1">Visualiza tu dirección real en las áreas clave de tu vida.</p>
        </div>
              ...
              {AREAS.map((area) => (
                <div key={area.id} className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-display font-bold text-primary">{newMetrics[area.id as keyof typeof newMetrics]}</span>
                  </div>
                  <Slider value={[newMetrics[area.id as keyof typeof newMetrics]]} onValueChange={([val]) => setNewMetrics({ ...newMetrics, [area.id]: val })} max={10} min={1} step={1} />
                </div>
              ))}
            </div>

            <div className="flex items-center space-x-2 bg-muted/30 p-4 rounded-xl border border-border/50">
              <Checkbox id="sync" checked={syncWithWheel} onCheckedChange={(checked) => setSyncWithWheel(!!checked)} />
              <div className="grid gap-1.5 leading-none">
                <label htmlFor="sync" className="text-sm font-medium leading-none flex items-center gap-2">
                  <RefreshCw className="w-3 h-3 text-primary" /> Sincronizar con la Rueda de la Vida
                </label>
                <p className="text-xs text-muted-foreground">Actualiza automáticamente tus puntajes del mes actual en el módulo de Identidad.</p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {AREAS.map((area) => {
          const lastValue = lifeMetrics.length > 0 ? lifeMetrics[lifeMetrics.length - 1][area.id as keyof typeof newMetrics] : '--';
          return (
            ...
            {lifeMetrics.length < 2 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} itemStyle={{ fontSize: '12px' }} />
                  <Legend />
                  {AREAS.map(area => (
                    <Line key={area.id} type="monotone" dataKey={area.id} name={area.label} stroke={area.color} strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                  ))}
                </LineChart>
              </ResponsiveContainer>
            )}
