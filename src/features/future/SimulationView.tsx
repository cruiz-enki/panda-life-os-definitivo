/**
 * **Vista** — Simulador de escenarios futuros (subvista de /future).
 */
import { useState } from "react";
import { useGrowth } from "@/hooks/use-growth";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Sparkles, TrendingUp, AlertTriangle, Calendar, ArrowRight, Loader2, History, Brain } from "lucide-react";
...
export function SimulationView() {
  const { runFutureSimulation, futureSimulations } = useGrowth();
  ...
  const handleSimulate = async () => {
    ...
    if (!result || result.error) toast.error("Error al generar la simulación");
    else toast.success("¡Simulación generada con éxito! 🔮");
  };
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-3">
            <Sparkles className="w-7 h-7 text-primary animate-pulse" /> Future Simulation
          </h2>
          <p className="text-muted-foreground text-sm mt-1">
            Proyecta tu futuro basándote en tus hábitos y métricas actuales.
          </p>
        </div>
        <Button onClick={handleSimulate} disabled={isSimulating} className="relative overflow-hidden group shadow-glow">
          {isSimulating ? (
      ...
      {!latestSimulation && !isSimulating && (
        <Card className="border-dashed border-primary/20 bg-primary/5">
          <CardContent className="p-12 flex flex-col items-center text-center space-y-4">
            ...
            <Button onClick={handleSimulate} variant="outline" className="mt-4">Comenzar primera simulación</Button>
          </CardContent>
        </Card>
      )}
                ...
                <Card key={idx} className={`overflow-hidden border-2 ${idx === 0 ? 'border-primary/10' : 'border-emerald-500/20 bg-emerald-500/5'}`}>
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground bg-muted px-2 py-1 rounded">{scenario.timeframe}</span>
                    </div>
                    <CardDescription className="text-sm mt-2 leading-relaxed">{scenario.summary}</CardDescription>
                  </CardHeader>
                    ...
                    <div className="space-y-3">
                      {scenario.projections.map((proj, pIdx) => (
                        <div key={pIdx} className="flex items-start gap-3 p-3 rounded-xl bg-background/50 border border-border/50">
                          <div className={`mt-1 font-bold text-lg ${proj.impact.includes('↑') ? 'text-emerald-500' : 'text-rose-500'}`}>{proj.impact}</div>
                          <div className="space-y-0.5">
                            <div className="text-sm font-bold flex items-center gap-2">{proj.category}</div>
                            <p className="text-xs text-muted-foreground leading-tight">{proj.description}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {latestSimulation.ai_insight && (
              <Card className="border-primary/20 bg-primary/5 shadow-glow overflow-hidden relative">
                ...
                <CardContent className="p-6 relative">
                  ...
                  <p className="text-lg font-display italic text-primary/90">"{latestSimulation.ai_insight}"</p>
                </CardContent>
              </Card>
            )}
          </motion.div>
          ...
          <div className="grid gap-3">
            {futureSimulations.slice(1, 5).map((sim) => (
              <div key={sim.id} className="flex items-center justify-between p-4 rounded-xl border border-border/50 bg-muted/20">
                ...
                <Button variant="ghost" size="sm" onClick={() => toast.info("Historial visualización en desarrollo")}>
