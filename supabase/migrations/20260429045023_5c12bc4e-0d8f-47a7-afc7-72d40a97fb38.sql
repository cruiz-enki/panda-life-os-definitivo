-- Habilitar extensiones (no-op si ya están)
CREATE EXTENSION IF NOT EXISTS pg_cron;
CREATE EXTENSION IF NOT EXISTS pg_net;

-- Función para construir el secret y URL desde vault (centraliza)
...
DECLARE
  cron_secret TEXT;
  base_url TEXT := 'https://project--ca32796b-30ed-4957-92e1-f3cbab31dd72.lovable.app';
  request_id BIGINT;
BEGIN
...
-- Función que arma y envía el resumen de tareas vencidas para usuarios cuyo notify_time = hora actual (en su tz)
...
BEGIN
  ...
  LOOP
    ...
    -- Llamamos al endpoint poll endpoint NO sirve aquí; usamos send via gateway directo no es posible desde SQL.
    -- En su lugar, llamamos a un endpoint dedicado (telegram-send) — lo creamos a continuación.
    PERFORM net.http_post(
      url := 'https://project--ca32796b-30ed-4957-92e1-f3cbab31dd72.lovable.app/api/public/hooks/telegram-send',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'x-cron-secret', cron_secret
      ),
...
-- Quitar jobs previos si existen (idempotente)
DO $$
BEGIN
  PERFORM cron.unschedule('telegram-polling');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

DO $$
BEGIN
  PERFORM cron.unschedule('telegram-overdue-tasks');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
...
SELECT cron.schedule(
  'telegram-polling',
  '* * * * *',
  $$ SELECT public._telegram_cron_call('/api/public/hooks/telegram-poll'); $$
);

-- Recordatorio de tareas vencidas cada hora en punto
SELECT cron.schedule(
  'telegram-overdue-tasks',
  '0 * * * *',
  $$ SELECT public._telegram_send_overdue_tasks(); $$
);